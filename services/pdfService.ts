...
export const generateFinancialReport = async (data: DashboardData, stats: any, forecast: any[]) => {
  ...
  const summaryData = [
    ["Metric", "Value", "Notes"],
    ["Deployable Funds", `PHP ${stats.deployableFunds?.toLocaleString() || 0}`, "True liquid spending power"],
    ["Monthly Overhead", `PHP ${stats.totalMonthlyCommitments?.toLocaleString() || 0}`, "Total monthly capital drain"],
    ["Safety Factor", `${stats.safetyFactorValue?.toFixed(0) || 0}%`, `Covers ${ ((stats.safetyFactorValue || 0) / 100).toFixed(1) } months of needs`],
    ["Total Debt", `PHP ${stats.totalDebtBalanceValue?.toLocaleString() || 0}`, "Aggregate active liabilities"],
    ["Vault Savings", `PHP ${stats.vaultSavings?.toLocaleString() || 0}`, "Secured reserve assets"]
  ];
  ...
  const forecastRows = (forecast || []).map(f => [
    f.name, 
    `PHP ${f.inflow?.toLocaleString() || 0}`, 
    `PHP ${f.outflow?.toLocaleString() || 0}`, 
    `PHP ${f.net?.toLocaleString() || 0}`, 
    `PHP ${f.balance?.toLocaleString() || 0}`
  ]);
